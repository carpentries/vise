...
verify_simple_vector <- function(inputs) {
  ...
  inputs_expr <- substitute(inputs)

  if (contains_function_calls(inputs_expr) || (any(funs) && any(pd$text[funs] != "c"))) {
    stop("::error::repository settings should be an R vector. No functions other than `c()` are allowed")
  }
  invisible()
}

contains_function_calls <- function(expr) {
  # not a call, all OK
  if (!is.call(expr)) {
    return(FALSE)
  }
  ...
  if (length(expr) > 1) {
    for (i in 2:length(expr)) {
      if (contains_function_calls(expr[[i]])) {
        return(TRUE)
      }
    }
  }
